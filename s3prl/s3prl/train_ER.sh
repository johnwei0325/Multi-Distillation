...
MODEL_NAME="humert_0.3_hubert_init_linear_2_layer_diff_translator"

# GPU device ID passed as an argument
GPU_DEVICE=$1

# Base command to train a model
BASE_CMD="python run_downstream.py -m train -u multi_distiller_local -d emotion -s paper -k result/pretrain/${MODEL_NAME}/states-epoch-12.ckpt -g result/pretrain/${MODEL_NAME}/config_model.yaml"
...
do
    ...
    EXP_NAME="${MODEL_NAME}_fewshot_20_ER_${i}"
    ...
    CMD="${BASE_CMD} -n ${EXP_NAME} -o config.downstream_expert.datarc.test_fold=fold${i}"
