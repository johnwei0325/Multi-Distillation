...
# Set the model name (e.g., wavlm+mert_librispeech)
...
BASE_CMD="python run_downstream.py -m train -u multi_distiller_local -d vocalset_technique_id -s paper -k result/pretrain/${MODEL_NAME}/states-epoch-12.ckpt -g result/pretrain/${MODEL_NAME}/config_model.yaml"

# Create an array to store test accuracy for each dataset
declare -a TEST_ACCURACIES

# Loop through 5 datasets with different directories for meta_data
for i in {1..5}
do
    # Set the root directory for each dataset
    DATASET_DIR="/home/johnwei743251/mdd/dataset/vocal_id/meta_data/few_shot_${i}"

    # Set a unique experiment name for each run (ensuring it's unique for each run)
    EXP_NAME="${MODEL_NAME}_fewshot_20_vocalID_${i}"

    # Set the CUDA_VISIBLE_DEVICES environment variable for this run
    export CUDA_VISIBLE_DEVICES=$GPU_DEVICE

    # Print dataset path and CUDA device for debugging
    echo "Training with dataset at: ${DATASET_DIR}"
    echo "Experiment Name: ${EXP_NAME}"
    echo "Using GPU: $CUDA_VISIBLE_DEVICES"

    # Set random seed for variability (optional)
    #RANDOM_SEED=$((RANDOM))
    ...
    CMD="${BASE_CMD} -n ${EXP_NAME} -o config.downstream_expert.datarc.meta_data='${DATASET_DIR}'"
