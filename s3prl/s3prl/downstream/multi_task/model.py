...
class SAP(nn.Module):
    ''' Self Attention Pooling module incorporate attention mask'''
    def __init__(self, out_dim):
...
class Model(nn.Module):
    def __init__(self, input_dim, agg_module, singer_output_dim, fluent_output_dim, config):
        ...
        self.singer_linear = nn.Linear(input_dim, singer_output_dim)
        self.fluent_linear = nn.Linear(input_dim, fluent_output_dim)
    ...
    def forward(self, features, att_mask):
        ...
        singer_pred = self.singer_linear(utterance_vector)
        fluent_pred = self.fluent_linear(utterance_vector)
        
        return singer_pred, fluent_pred, utterance_vector 
