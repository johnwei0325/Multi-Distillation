...
def get_downstream_model_multi_task(input_dim, output_dim_1, output_dim_2, config):
    model_cls = eval(config['select'])
    model_conf = config.get(config['select'], {})
    model = model_cls(input_dim, output_dim_1, output_dim_2, **model_conf)
    return model
...
class FrameLevelMultiTask(nn.Module):
    def __init__(self, input_dim, output_dim_1, output_dim_2, hiddens=None, activation='ReLU', **kwargs):
        super().__init__()
        latest_dim = input_dim
        self.hiddens = []
        ...
        if hiddens is not None:
            for dim in hiddens:
                ...
                latest_dim = dim
        self.hiddens = nn.Sequential(*self.hiddens)
        self.linear_1 = nn.Linear(latest_dim, output_dim_1)
        self.linear_2 = nn.Linear(latest_dim, output_dim_2)

    def forward(self, hidden_state, features_len=None):
        hidden_state = self.input_norm(hidden_state)
        hidden_state = self.hiddens(hidden_state)
        logit_1 = self.linear_1(hidden_state)
        logit_2 = self.linear_2(hidden_state)

        return (logit_1, logit_2), features_len
...
class UtteranceLevelMultiTask(nn.Module):
    def __init__(self,
        input_dim,
        output_dim_1,
        output_dim_2,
        ...
        activation='ReLU',
        pre_net=None,
        post_net={'select': 'FrameLevelMultiTask'},
        **kwargs
    ):
        super().__init__()
        latest_dim = input_dim
        self.pre_net = get_downstream_model(latest_dim, latest_dim, pre_net) if isinstance(pre_net, dict) else None
        self.pooling = eval(pooling)(input_dim=latest_dim, activation=activation)
        self.post_net = get_downstream_model_multi_task(latest_dim, output_dim_1, output_dim_2, post_net)
    ...
    def forward(self, hidden_state, features_len=None):
        ...
        pooled, features_len = self.pooling(hidden_state, features_len)
        (logit_1, logit_2), features_len = self.post_net(pooled, features_len)
...
class AttentivePoolingModule(nn.Module):
    ...
    def forward(self, batch_rep, att_mask):
        ...
        utter_rep: size (B, H)
        """
        batch_rep = self.layer_norm(batch_rep)
        att_logits = self.W(self.act_fn(self.W_a(batch_rep))).squeeze(-1)
        
        att_logits = att_logits / (att_logits.size(-1) ** 0.5)
