...
BASE_CMD="python run_downstream.py -m train -u multi_distiller_local -d vocalset_singer_id -s paper -k result/pretrain/${MODEL_NAME}/states-epoch-12.ckpt -g result/pretrain/${MODEL_NAME}/config_model.yaml"
...
do
    ...
    DATASET_DIR="/home/johnwei743251/mdd/dataset/singer_id/meta_data/few_shot_${i}"

    # Set a unique experiment name for each run (ensuring it's unique for each run)
    EXP_NAME="${MODEL_NAME}_fewshot_20_singerID_${i}_test2"
